...
void loop() {
  ...
  if ( (millis() - last_broadcast) > 1000*5 ) {
...
/*
  if (readButton()) {
    //LED.code(LED.WARNING);
    if ( millis() - start_press_duration > 5000) {
      wifiManager.resetSettings();
      ESP.restart();
      delay(1000);
    }
  } else {
    start_press_duration = millis();
  }
  */
}
...
uint8_t readLightLevel () {
  uint8_t _value = map( analogRead(LIGHT_LEVEL_PIN), 0, 1023, 0, 100);

  light_level_max = light_level_max < _value ? _value : light_level_max;
  light_level_min = light_level_min < _value ? _value : light_level_min;
